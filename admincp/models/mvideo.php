...
class Models_MVideo extends Project
{
	...
	function listdata(){
		$this->getdata(TBL_VIDEO,"sort asc, Id desc");
	...
	function delFile($data,$id){
		$this->where('Id = '.$id);
		$this->update(TBL_VIDEO,$data);	
	}
	
	//them mot chu de
	function addflash($data){
		$this->insert(TBL_VIDEO,$data);
		if($this->result == 1){
			return 1;
		}else{
			return 0;
	...
	function del_allcheck($data){
		foreach($data as $item)
		{
			...
			$this->delete(TBL_VIDEO,'Id = '.$item);
		}
	}
	
	//delete mot dong
	function del_onecheck($id){
		$this->delete(TBL_VIDEO,'Id = '.$id);
	}
	...
	function sortData($data){
		foreach($data as $key => $value){
			$this->where("Id = $key");
			$this->update(TBL_VIDEO,"sort = $value");
		}
	}
}
?>